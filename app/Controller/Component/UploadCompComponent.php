...
class UploadCompComponent extends Component {
	 ...
	 * @param $config array
	 */
	function initialize(Controller $controller, $config=array()) {
	...
	*/
	...
	function upload_FS(&$data) {
		...
		$filename = reset(split('\.', $data['name']));
		...
		if ($this->is_image($filetype)) {
			$this->copy_or_log_error($data['tmp_name'], $tmpuploaddir, $filename);
			...
			if (!$this->config['allow_non_image_files']) {
				...
				@mkdir($fileuploaddir);
				//pr($data);
				//echo $fileuploaddir."<br />";
				//echo $filename."<br />";
				//exit;
				$this->copy_or_log_error($data['tmp_name'], $fileuploaddir, $filename);
		...
		//$res[$column_prefix.'_file_name'] = $data['name'];
		//$res[$column_prefix.'_file_size'] = $data['size'];
		//$res[$column_prefix.'_content_type'] = $data['type'];
		//unset($data[$column_prefix]); /* delete $_FILES indirection */
		//$data = array_merge($data, $res); /* add default fields */
		//#######################################################
		//pr($this->controller->data);
		//pr($data);
		//exit;
		$res[$column_prefix.'_file_path'] = $filename;
		$res[$column_prefix.'_file_name'] = $data['name'];
	...
	function delete_files($filename) {
		/* Non image files */
		//echo WWW_ROOT.'files'.DS.'files'.DS.$filename;
		
		/*if (is_file(WWW_ROOT.'files'.DS.'files'.DS.$filename)) {
			unlink(WWW_ROOT.'files'.DS.'files'.DS.$filename);
		}*/
		/* tmp files (if not pruned while uploading) */
		/*if (is_file(WWW_ROOT.'files'.DS.'tmp'.DS.$filename)) {
			unlink(WWW_ROOT.'files'.DS.'tmp'.DS.$filename);
		}*/
		/* Thumbnail copies */
